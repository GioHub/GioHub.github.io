...
const AISummary = (() => {
  ...
  function generate(records, metrics, aggregates, activeFilters, risks = []) {
    if (metrics.total === 0) {
    ...
    if (metrics.overdue > 0) {
      sentences.push(`${metrics.overdue} work item(s) tienen Target Date vencido sin estar cerrados.`);
    }
    ...
    if (risks && risks.length) {
      const riskTexts = risks.map(r => r.text.replace(/^Riesgo[a-záéíóúñ ]*: /i, '').replace(/^./, c => c.toUpperCase()));
      sentences.push(`Riesgos identificados por el motor de reglas: ${riskTexts.join(' ')}`);
    }
